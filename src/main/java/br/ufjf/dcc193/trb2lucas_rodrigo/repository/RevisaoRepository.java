...
public interface RevisaoRepository extends JpaRepository<Revisao, Integer> {
    ...
    @Query("Select new br.ufjf.dcc193.trb2lucas_rodrigo.DTO.TrabalhoDTO(t.id, t.titulo,t.descricao,t.url,t.area,0L) from Trabalho t left join Revisao r on r.trabalho.id = t.id where t.area = :area and (r.status.id <> 1 or r.status = null) ")
